...
# Default 4 Year Wardens - Admin mattum set pannum (Mobile number included)
DEFAULT_WARDENS = {
...
def load_users():
    ...
    # First time la default wardens create pannuvom
    if not users:
        default_list = []
...
.badge{padding:3px 9px;border-radius:20px;font-size:11px;color:white;} .b-pending{background:#f59e0b;} .b-ok{background:#16a34a;} .b-no{background:#dc2626;}
</style>
"""
...
def home():
    ...
    <div class="container">
        ...
        <div class="card"><b>Note:</b> 4 Year Wardens are created by Admin only. New users can register as Gate Security or Admin (needs Admin approval). Passwords are hidden and secure.</div>
    </div></body></html>
    """)

# REGISTER - Mobile number mandatory
...
def register():
    if request.method == 'POST':
        ...
        users.append({
            'id': len(users)+1, 'username': uname, 'password': request.form['password'],
            'role': request.form['role'], 'year': request.form.get('year','All'),
            'mobile': request.form['mobile'], 'status': 'Active' if request.form['role']!='admin' else 'Pending'
        })
        save_json(USERS_FILE, users)
        return render_template_string(f"<html><head>{BASE}</head><body><div class='container'><div class='card' style='text-align:center;'><h3 style='color:green;'>Account Created!</h3><p>Username: {uname} - Mobile: {request.form['mobile']}</p><a href='/login' class='btn'>Login Now</a></div></div></body></html>")
    return render_template_string(f"""
    <html><head><title>Register</title>{BASE}</head><body>
    <div class="header"><h2>Create Account</h2></div>
    <div class="container"><div class="card" style="max-width:500px;margin:auto;"><form method="POST">
    <label>Username</label><input name="username" required>
    <label>Mobile Number (for password recovery)</label><input name="mobile" pattern="[0-9]{{10}}" placeholder="10 digit mobile" required>
    <label>Role</label><select name="role" required><option value="gate">Gate Security</option><option value="admin">Admin</option><option value="warden">Warden (Only Admin can create Year Wardens)</option></select>
    <label>If Warden - Select Year</label><select name="year"><option>1st Year</option><option>2nd Year</option><option>3rd Year</option><option>Final Year</option><option>All</option></select>
    <label>Password</label><input type="password" name="password" required>
    <button class="btn" style="width:100%;">Create Account</button></form></div></div></body></html>
    """)
...
def forgot_password():
    if request.method == 'POST':
        ...
        # If warden wants to change password - needs admin permission
        if user['role']=='warden':
            ...
            return "<div style='text-align:center;padding:40px;font-family:Arial;'><h3 style='color:orange;'>Password reset request sent to Admin for approval</h3><p>Admin will approve your new password shortly</p><a href='/'>Home</a></div>"
        else:
            # Gate/Admin direct reset via mobile verification
    ...
    return render_template_string(f"""
    <html><head><title>Forgot Password</title>{BASE}</head><body>
    <div class="header"><h2>Forgot Password - Recover via Mobile</h2></div>
    <div class="container"><div class="card" style="max-width:500px;margin:auto;"><form method="POST">
    <label>Username</label><input name="username" required>
    <label>Registered Mobile Number</label><input name="mobile" pattern="[0-9]{{10}}" required>
    ...
    <p style="font-size:12px;color:#666;">Warden password change needs Admin permission. Gate/Admin can reset directly via mobile verification.</p>
...
def login():
    if request.method == 'POST':
        ...
        if not user: return "<h3 style='text-align:center;color:red;'>Invalid login or account pending <a href='/login'>Try again</a></h3>"
        session['user']=user
    ...
    return render_template_string(f"""
    <html><head><title>Login</title>{BASE}</head><body>
    <div class="header"><h2>Login - Warden / Gate / Admin</h2></div>
    <div class="container"><div class="card" style="max-width:450px;margin:auto;"><form method="POST">
    <label>Username</label><input name="username" required><label>Password</label><input type="password" name="password" required>
    <button class="btn" style="width:100%;">Login</button></form><br><a href="/forgot-password">Forgot Password?</a> | <a href="/register">Create Account</a></div></div></body></html>
    """)
...
def student_outpass():
    if request.method == 'POST':
        ...
        data.append({
            'id':len(data)+1,'name':request.form['name'],'roll_no':request.form['roll_no'],'phone':request.form['phone'],
            'year':request.form['year'],'reason':request.form['reason'],'from_date':request.form['from_date'],'to_date':request.form['to_date'],
    ...
    return render_template_string(f"""
    <html><head><title>Outpass</title>{BASE}</head><body>
    <div class="header"><img src="/logo.png" class="logo"><h2>Student Outpass</h2></div>
    <div class="container"><div class="card" style="max-width:600px;margin:auto;"><form method="POST">
    <label>Name</label><input name="name" required><label>Roll No</label><input name="roll_no" required>
    <label>Phone</label><input name="phone" required><label>Year</label><select name="year" required><option>1st Year</option><option>2nd Year</option><option>3rd Year</option><option>Final Year</option></select>
    <label>From</label><input type="date" name="from_date" required><label>To</label><input type="date" name="to_date" required>
    <label>Reason</label><textarea name="reason" required></textarea><button class="btn" style="width:100%;">Submit</button></form></div></div></body></html>
    """)
...
def student_status():
    ...
    return render_template_string(f"""
    <html><head><title>Status</title>{BASE}</head><body>
    <div class="header"><h2>Check Outpass Status</h2></div>
    <div class="container"><div class="card" style="max-width:600px;margin:auto;"><form method="POST">
    <input name="search" placeholder="Phone or Roll No" required><button class="btn" style="width:100%;">Search</button></form></div>{html_result}
...
# GATE DASHBOARD - With Returned View
...
def gate_returned(id):
    ...
    data=load_json(OUTPASS_FILE)
    for r in data:
        if r['id']==id: r['returned']='Yes - Returned on '+datetime.now().strftime("%Y-%m-%d %H:%M")
...
def admin_dashboard():
    ...
    reset_rows="".join([f"<tr><td>{r['id']}</td><td>{r['username']}</td><td>{r['mobile']}</td><td>{r['status']}</td><td><a href='/admin/approve-reset/{r['id']}' class='btn btn-green' style='padding:4px 8px;'>Approve</a></td></tr>" for r in resets if r['status']=='Pending']) or "<tr><td colspan='5'>No pending reset requests</td></tr>"
    return render_template_string(f"""
    <html><head>{BASE}</head><body><div class="header"><h2>Admin Dashboard</h2><a href="/logout" style="color:white;">Logout</a> | <a href="/" style="color:white;">Home</a></div>
    <div class="container">
        <div class="card"><h3>Password Reset Requests (Warden needs Admin Permission)</h3><table><tr><th>ID</th><th>Username</th><th>Mobile</th><th>Status</th><th>Action</th></tr>{reset_rows}</table></div>
        <div class="card"><h3>All Users (Passwords Hidden - Secure)</h3><table><tr><th>ID</th><th>Username</th><th>Role</th><th>Year</th><th>Mobile</th><th>Status</th></tr>{user_rows}</table></div>
        <div class="card"><h3>Recent Outpass (Last 20)</h3><table><tr><th>ID</th><th>Student</th><th>Warden Status</th><th>Gate</th><th>Returned</th></tr>{out_rows}</table></div>
    </div></body></html>
    """)
